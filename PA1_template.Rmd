...
#head(complete.data)
#first plot without averaging across days
#with(complete.data, plot(steps ~ interval, type="l"))

#split the data on time intervals
#interval.data <- split(data, data$interval)
#intervalMeanSteps <- unlist(lapply(interval.data, function(x) mean(x$steps, na.rm=TRUE)))
#head(intervalMeanSteps)
#intervals <- names(intervalMeanSteps)
#length(intervals)
